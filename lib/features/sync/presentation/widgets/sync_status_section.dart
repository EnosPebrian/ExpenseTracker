...
class SyncStatusSection extends StatelessWidget {
  ...
  final CloudSyncDecision? cloudDecision;
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        final classification = cloudDecision?.classification;
        final effectiveStatus = switch (classification) {
        ...
        final hideSyncHistory =
            classification == CloudSyncClassification.remoteStateChecking ||
            classification ==
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              Text(
                classification == CloudSyncClassification.remoteStateChecking
                    ? 'Checking cloud household…'
                    : classification ==
                          CloudSyncClassification.reconnectSameHostedHousehold
                    ? 'Reconnect cloud sharing'
                    : _label(effectiveStatus, controller.pendingCount),
                key: const Key('sync-status-label'),
                style: const TextStyle(fontWeight: FontWeight.w600),
              ),
              if (effectiveStatus == SyncStatus.primaryUploadRequired &&
                  !hideSyncHistory) ...[
              ...
              if (effectiveStatus == SyncStatus.secondaryDownloadRequired) ...[
                const SizedBox(height: 4),
                const Text(
