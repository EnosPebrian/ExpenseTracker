...
- Added the in-memory versioned `TithePolicy`; `FinancialSummary` and AppShell now resolve tithe rates by effective date.
- Added FP-021 Net Worth Dashboard analytics: total assets, liabilities, net worth, cash availability, pending-tithe impact, dynamic asset allocation, and liability totals.
...
- Add later tithe feature packs for payment tracking, carry-forward, persistence, and rule editing.
- FP-021 intentionally leaves asset management, persistence, and valuation workflows for later feature packs.
